...
- **9.10 (MENUCARD)** — **Names that match the game's own menus.** Every Timeloop now carries its exact in-game name, even where that name just repeats its region — looking an entry up while the game is open beats avoiding a little redundancy, and the invented spot names ("Beyond the Bridge") made that impossible. Each Timeloop's description now opens with the Amine Prism it needs, in the accent colour, instead of burying it in a closing sentence. Two Timeloops where the guides genuinely disagree list both prisms and say so. In the By-category list, the sub-area in brackets is now hidden on the few entries where it only repeated the entry's own name — it still shows wherever it tells you something the name doesn't. Also added: the 30th relic, **Mark of the Griffon**, which Deluxe Edition owners find in the APC — it's listed so the app matches the in-game relic list, clearly marked as Deluxe-only, and deliberately left out of every count, so Antiquarian still reads out of 29 and standard-edition players know they're not missing anything.
- **9.9.1 (ATLAS)** — **Layout polish on the new place labels.** Row titles no longer break awkwardly onto a second line — a place label that doesn't fit drops down whole, on both tabs. The zone sub-headers in the By-location view now stand out in the interface accent with an underline instead of blending in with the labels around them.
- **9.9 (ATLAS)** — **Place names now match the game's map.** Every place in the app was renamed to the official Location names from the game's world (per the community wiki), and the checklist now knows the map's real hierarchy: the Completionist "By location" view groups everything under the twelve Locations, with landmarks like the Lymbic Forge, Pathem Abbey, and the Vaults shown as labelled sub-areas within them. The Campaign rows use the same grammar — *Acasa Marshes (Village of Jova)* — and a few spellings were corrected against the wiki.
- **9.8 (RELAY)** — **Step-by-step tracking for cross-region deeds.** In the Completionist's By-zone view, a deed or mystery that spans regions now appears as separate steps — a *(step 1 of 2)* pickup where you start it and the completion where you finish it — so ticking a step in one region no longer marks the whole thing done before you've delivered. Ticking the deed anywhere (or in By-category) still lights up all its steps, and a two-delivery deed like Mementos only completes once *both* handoffs are checked. Plus a descriptions cleanup: cross-references now name the actual deed or mystery ("the Depths of Hell good deed") instead of a bare number.
- **9.7.2 (MILLINER)** — **Completionist list tidy-up.** The collectible lists now read name-first like the campaign checklist: a chest or door leads with what it is, and the specific spot drops to a smaller note in parentheses — so *"Lymbic Chest – Acasa, Vitalis' Basement"* becomes **Lymbic Chest** *(Vitalis' Basement)*, with the region already shown by the location label beside it. Fourteen chest/door entries were cleaned up this way, and every Timeloop was renamed to a distinct spot — no more entries whose name just echoed the region shown beside them.
- **9.7.1 (MILLINER)** — **Campaign polish.** The coloured tags (MISSABLE, STORY, WARNING…) are now all one uniform width, so every row's title lines up in a clean column down the page. A short note in parentheses — like *(1 of 2)* — no longer splits across two lines; the whole note stays together and bumps down as a unit. The collapse arrows and row expanders are a touch larger and easier to tap. Plus small wording fixes on the Atticus vehicle-keys and B1-switch cautions.
